<template>
    ...
    <div id="mapWrapper2" :style="wrapperStyle">
      <div v-for="(row, rowIndex) in blocks" :key="'map_row_' + rowIndex" class="row">
          <img v-for="(block, blockIndex) in row"
                  :key="'map_block_' + rowIndex + '_' + blockIndex"
                  class="block"
                  alt=""
                  :src="printTransparent(block)"
          />
      </div>
    </div>
</template>
...
import planta_transparente from '@/assets/map/high_grass_transparent.png';
...
export default {
    ...
    methods: {
        ...
        printTransparent(blockType) {
            return blockType == 4? planta_transparente : '';
        },
